...
class Battleship {
  ...
  StartGame() {
    ...
    do {
      ...
      if (coinFlip) {
        ...
        computerPos = this.GetGuaranteedHitPosition(); // find a guaranteed hit;
      }
      while (
        computerGuesses.find(
          (guess) =>
            guess.row === computerPos.row && guess.column === computerPos.column
        )
      ) {
        computerPos = this.GetRandomPosition();
      }
  ...
  PrintEnemyFleetStatus() {
    ...
    this.enemyFleet.forEach((ship) => {
      console.log(
        `${ship.name} (${ship.size}): ${
          ship.isSunk() ? cliColor.green("Sunk") : cliColor.red("Not Sunk")
        }`
      );
    });
  }
  ...
  CheckWinCondition() {
    ...
    if (humanPlayerWon) {
      console.log(
        cliColor.green(
          "Congratulations!! You won the war against the evil Milton"
        )
      );
    }
    if (cpuPlayerWon) {
  ...
  GetGuaranteedHitPosition() {
    let positionToHit = this.myFleet.forEach((ship) => {
      if (!ship.isSunk()) {
        return ship.positions.filter((position) => !position.isHit);
      }
    });
    var result = new position(letter, number);
  }
  ...
  InitializeMyFleet() {
    ...
    this.myFleet.forEach(function (ship) {
      ...
      for (var i = 1; i < ship.size + 1; i++) {
        console.log(
          cliColor.yellow(`Enter position ${i} of ${ship.size} (i.e A3):`)
        );
